...
best_threshold = {4:0.015170,6:0.014256,12:0.011668,24:0.012105,32:0.017430,\
                  64:0.028987,96:0.032550,192:0.059358,288:0.100416}
    ...
    for rack in racks:
        ...
        for idx,ts in enumerate(ts_list):
            ...
            for i in range(len(gnn_pred)):
                thresh = best_threshold[f_w]
                # print(thresh)
                if gnn_pred[i].item()>thresh:
                    gnn_pred_list.append(1)
                else:
                    gnn_pred_list.append(0)
            ...
            for i,filename in enumerate(files):
                ...
                # Random Forest
                rf_prob = rf_model.predict(feat_vector)[0]
                # rf_preds.append(rf_probability)
                # print(f"RF: {rf_prob[0]}")

                # Support Vector Machine
                svm_prob = svm_model.predict(feat_vector)[0]
                # svm_preds.append(svm_probability)
                # print(f"SVM: {svm_prob[0]}")

                # Decision Tree
                dt_prob = dt_model.predict(feat_vector)[0]
                # dt_preds.append(dt_probability)
                # print(f"DT: {dt_prob[0]}")

                # Logistic Regression
                lr_prob = lr_model.predict(feat_vector)[0]
                # lr_preds.append(lr_probability)
                # print(f"LR: {lr_prob[0]}")

                # **Hard Voting:** Take majority class votes
                # Stack predictions
                stacked_preds = np.column_stack((gnn_prob, int(rf_prob), int(svm_prob), int(dt_prob), int(lr_prob)))
                hard_voting_preds = np.apply_along_axis(lambda x: np.bincount(x).argmax(), axis=1, arr=stacked_preds)

                hard_probs.append(hard_voting_preds[0])
                # print(stacked_preds)

                # print(hard_voting_preds[0])

    print(ground_truths)
    print(hard_probs)
    # Evaluate ROC-AUC for hard voting
    roc_auc_hard = roc_auc_score(ground_truths, hard_probs)
    print("Hard Voting ROC-AUC:", roc_auc_hard)

    auc_fw.append(roc_auc_hard)
