...
for rack in racks:
    for f_w in f_ws:
        print(f"Rack: {rack} | Future prediction(hours) : {f_w/4}")
        ...
        for i in range(len(files)):
            ...
            lr.fit(train_feat,train_label)

            y_pred = lr.predict_proba(test_feat)[:, 1]
            ...
            os.makedirs(f"ensemble_models/LR/{f_w}", exist_ok=True)

            filename = 'ensemble_models/LR/{}/{}_{}.pickle'.format(f_w,rack,get_nodename(files[i]))
